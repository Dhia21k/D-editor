...
char* C_HL_keywords[] = {
	"switch", "if", "while", "for", "do", "break", "continue", "return", "else",
	"struct", "union", "enum", "typedef", "static", "class", "case",
...
#define HLDB_ENTERIES (sizeof(HLDB) / sizeof(HLDB[0]))
...
void initEditor() {
	...
	if (getWindowSize(&E.screenrows, &E.screencols) == -1) 
		die("getWindowSize");
	// Making room for a one line status bar
	E.screenrows -= 2;
}
...
void editorSelectSyntaxHighlight() {
	...
	for (unsigned int j = 0; j < HLDB_ENTERIES; j++) {
		...
		while (s->filematch[i]) {
			...
			if ((is_ext && ext && !strcmp(ext, s->filematch[i])) ||
				(!is_ext && strstr(E.filename, s->filematch[i]))) {
					E.syntax = s;
					int filerow;
					// Updating the whole file with syntax highliting
...
void editorUpdateRow(erow* row) {
	...
	free(row->render);
	row->render = malloc(row->size + tab * (TAB_STOP - 1) + 1);
...
void editorInsertRow(int at, char* s, size_t len) {
	...
	// Initializing the render element
	E.row[at].rsize = 0;
...
void editorFindCallback(char* query, int key) {
	...
	if (last_match == -1) direction = FORWARD;
...
void editorProcessKeypress() {
	...
	switch (c) {
		...
		case CTRL_KEY('q'):
			...
			write(STDOUT_FILENO, "\x1b[2J", 4);
			write(STDOUT_FILENO, "\x1b[H", 3);
			// Freeing the rows stored in the editor
			for (int i = 0; i < E.numrows; i++) {
